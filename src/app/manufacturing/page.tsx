...
export async function generateMetadata(): Promise<Metadata> {
  const page = await client.fetch(getPageQuery, { slug: "manufacturing" });
  if (!page) return { title: "Manufacturing" };
  
  return {
    title: page.seoTitle || page.title || "Manufacturing",
    description: page.seoDescription || "Our Manufacturing Process",
  };
}
    ...
    image: ({ value }: { value: unknown }) => {
      ...
      return (
        <div className="relative h-64 md:h-96 w-full my-8">
          <Image
            src={urlFor(val).url()}
            alt={val.alt || "Manufacturing Image"}
            fill
  ...
  if (!page) {
    return (
      <div className="container mx-auto px-4 md:px-8 py-20 md:py-[120px] min-h-[60vh] flex flex-col justify-center">
        <span className="text-primary font-semibold tracking-widest uppercase text-sm mb-4 block">Process</span>
